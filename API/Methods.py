...
@api.route('/admin/news/get',methods=['GET'])
def admin_news_get():
    return json.dumps(DB().selectFromDB("SELECT * FROM \"NEWS\" WHERE site_id = 1"))

@api.route('/admin/news/add', methods=['POST'])
def admin_news_add():
    try:
...
@api.route('/admin/news/change', methods=['POST'])
...
@api.route('/admin/news/remove',methods=['POST'])
